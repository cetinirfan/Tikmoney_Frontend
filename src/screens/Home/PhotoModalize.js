...
@inject("AuthStore")
@observer
export default class PhotoModalize extends Component {
  constructor(props) {
    super(props);
    this.state = {
      image: null,
  ...
  pickSingleWithCamera = (cropping, mediaType = 'photo') => {
    ...
    ImagePicker.openCamera({
      ...
      .then((image) => {
        ...
        this.props.selectedPhoto(image);
        this.onClose();
        this._Save(data1)
      })
      .catch((e) => alert(e));
  ...
  _Save = async (data1) => {
    try {
      ...
      const {data} = await axios.post(`${SERVER_URL}/users/upload`,data1,{ 'headers': { 'x-access-token': token }});
      if (data.status) {
        ...
        Alert.alert(
          ...
          data.message[1],
          [
            { text: I18n.locale === 'tr-TR' ? 'Tamam' : 'OK' ,onPress: async () => {this.props.onPressProfile()}, style: 'cancel'},
          ],
          { cancelable: false }
        );
      }
